...
class SelectGameBloc extends Bloc<SelectGameEvent, SelectGameState> {
  ...
  double _index;
  SelectGameBloc({required this.listGameName, required double initialIndex})
      : _index = initialIndex,
  ...
  FutureOr<void> _changeGameSelect(
    SelectGameChange event,
    Emitter<SelectGameState> emit,
  ) {
  ...
  FutureOr<void> _loadGames(
    SelectGameloadList event,
    Emitter<SelectGameState> emit,
  ) {
  ...
  List<TileImagePre> filteredImageByGame() {
    return state.runtimeType == SelectGameChanged
        ? listImagePre
            .where(
              (element) =>
                  element.gameName ==
                  listCardGame[(state as SelectGameChanged).index.toInt()]
                      .gameName,
            )
            .toList()
        : [];
