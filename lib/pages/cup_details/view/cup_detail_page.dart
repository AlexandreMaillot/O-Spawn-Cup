...
class CupDetailPage extends StatelessWidget {
  ...
  const CupDetailPage({Key? key, required this.tournament}) : super(key: key);
  Route route() {
    return MaterialPageRoute<void>(
      builder: (_) => CupDetailPage(
  ...
  Widget build(BuildContext context) {
    ...
    final teamRepository = TeamRepository(
      teamCollectionReference: TeamCollectionReference(
        tournamentsRef.doc(tournament.documentId).reference,
      ),
    );
    final cupDetailCubit = CupDetailCubit(
      tournamentRepository: tournamentRepository,
      teamRepository: teamRepository,
      appBloc: context.read<AppBloc>(),
      memberRepository: MemberRepository(
        memberCollectionReference: m.MemberCollectionReference(),
      ),
