...
FormBlocStep buildStep1(
  SelectGameBloc selectGameBloc,
  TournamentFormBloc tournamentFormBloc,
) {
  tournamentFormBloc.pageController.addListener(() {
    tournamentFormBloc.game.updateValue(
      listCardGame[tournamentFormBloc.pageController.page!.toInt()].gameName,
    );
    selectGameBloc.add(
      SelectGameChange(indexSelect: tournamentFormBloc.pageController.page!),
    );
  });
  const tauxScale = 0.75;

  return FormBlocStep(
    state: tournamentFormBloc.state.currentStep > 0
        ? StepState.complete
        : StepState.disabled,
        ...
        return BlocBuilder<SelectGameBloc, SelectGameState>(
          builder: (context, state) {
            if (state.runtimeType == SelectGameNoData) {
              ...
              final indexSelect = context.select(
                (SelectGameBloc bloc) =>
                    (bloc.state as SelectGameChanged).index,
              );
                ...
                child: PageView.builder(
                  ...
                  itemBuilder: (context, position) {
                    if (position == indexSelect) {
                      ...
                      return Transform.scale(
                        scale: max(1 - (indexSelect - position), tauxScale),
                        child: GameCard(
                          position,
                          form: true,
                        ),
                      ...
                      return Transform.scale(
                        scale: max(1 - (position - indexSelect), tauxScale),
                        child: GameCard(
                          position,
                          form: true,
                        ),
  ...
  WidgetsBinding.instance!.addPostFrameCallback((_) {
    if (tournamentFormBloc.pageController.hasClients) {
      if (tournamentFormBloc.tournament != null) {
        final indexGame = listCardGame.indexWhere(
          (element) => element.gameName == tournamentFormBloc.tournament!.game,
        );
