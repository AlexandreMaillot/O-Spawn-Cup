...
void main() {
  ...
  group('Authenticated', (){
    test('authenticated', () {
      var app = AppState(status: AppStatus.authenticated,user: user,);
      expect(
    ...
    test('gameSelect', () {
      var app = AppState(status: AppStatus.authenticated,user: user,gameName: gameName);
      expect(
        onGenerateAppViewPages(app: app,pages: []),[
        isA<MaterialPage>().having((p) => p.child, '', isA<Home>()),
    ...
    test('tournamentSelect', () {
      var app = AppState(status: AppStatus.authenticated,user: user,gameName: gameName,tournament: tournament);
      expect(
        onGenerateAppViewPages(app: app),[
        isA<MaterialPage>().having((p) => p.child, '', isA<Home>()),
        isA<MaterialPage>().having((p) => p.child, '', isA<ListCupPage>()),
    ...
    test('form tournament null', () {
      var app = AppState(status: AppStatus.authenticated,user: user,gameName: gameName,tournament: tournament,formTournamentStatus: null);
      expect(
        onGenerateAppViewPages(app: app),[
        isA<MaterialPage>().having((p) => p.child, '', isA<Home>()),
        isA<MaterialPage>().having((p) => p.child, '', isA<ListCupPage>()),
    ...
    test('form tournament en creation', () {
      var app = AppState(status: AppStatus.authenticated,user: user,gameName: gameName,tournament: tournament,formTournamentStatus: FormTournamentStatus.enCreation);
      expect(
        onGenerateAppViewPages(app: app),[
        isA<MaterialPage>().having((p) => p.child, '', isA<Home>()),
        isA<MaterialPage>().having((p) => p.child, '', isA<FormTournament>().having((p0) => p0.tournament, 'tournois', null)),
      ],);
    });
    test('form tournament en modification', () {
      var app = AppState(status: AppStatus.authenticated,user: user,gameName: gameName,tournament: tournament,formTournamentStatus: FormTournamentStatus.enModification);
      expect(
        onGenerateAppViewPages(app: app),[
        isA<MaterialPage>().having((p) => p.child, '', isA<Home>()),
        isA<MaterialPage>().having((p) => p.child, '', isA<FormTournament>().having((p0) => p0.tournament, 'tournois', tournament)),
      ],);
    });
  });
