...
void main() {
  ...
  late MockFirebaseStorage firebaseStorageMock;
  ...
  setUp((){
    ...
    firebaseStorageMock = MockFirebaseStorage();
  ...
  group('Normal test initCup', (){
    ...
    test('InitCup', () {
      ...
      DateTime nextDate = DateTime(now.year,now.month,now.day + 7);
      when(() => tournament.game,).thenReturn(GameName.LeagueOfLegend);
      when(() => tournament.name,).thenReturn('MonTournois');
      when(() => tournament.roundNumber,).thenReturn(5);
      when(() => tournament.capacity,).thenReturn(32);
      when(() => tournament.dateDebutInscription,).thenReturn(now);
      when(() => tournament.dateDebutTournois,).thenReturn(nextDate);
      when(() => tournament.tournamentType,).thenReturn(TournamentType.trio);
      when(() => tournament.server,).thenReturn(ServerType.EU);
      when(() => tournament.killPointTournament,).thenReturn(1);
      when(() => tournament.pointPerRangTournament,).thenReturn(2);
      when(() => tournament.rangStartTournament,).thenReturn(5);
      when(() => tournament.imageName,).thenReturn('123456');
      when(() => firestorageService.downloadFileImage(any()),).thenAnswer((invocation) => Future.value(file));
      when(() => file.path,).thenReturn('monpath');
      when(() => tournament.cashPrize,).thenReturn(['lot1','lot2']);
      when(() => tournament.listCodesGames,).thenReturn(['147','258','369']);

      // tester.binding.addPersistentFrameCallback((timeStamp) {
      // });
      var mytournamentFormBloc = TournamentFormBloc(tournamentRepository: TournamentRepository(tournamentCollectionReference: tournamentCollection),
        widgetNumberByPlayerBloc: widgetNumberByPlayerBloc,
        selectedImagePredefCubit: selectedImagePredefCubit,
        takeImageGalleryCubit: takeImageGalleryCubit,
        generateCodeCubit: generateCodeCubit,
      ...
      expect(mytournamentFormBloc.gameNumber.value, tournament.roundNumber.toString());
      expect(mytournamentFormBloc.signCupDate.value, tournament.dateDebutInscription);
      expect(mytournamentFormBloc.startCupDate.value, tournament.dateDebutTournois);
      expect(mytournamentFormBloc.numberPlayerByTeam.value, tournament.tournamentType);
      expect(mytournamentFormBloc.startTime.value, TimeOfDay.fromDateTime(tournament.dateDebutTournois!));
      expect(mytournamentFormBloc.numberTeam.value, tournament.capacity.toString());
      expect(mytournamentFormBloc.server.value, tournament.server);
      expect(mytournamentFormBloc.pointPerKill.value, tournament.killPointTournament.toString());
      expect(mytournamentFormBloc.pointPerRang.value, tournament.pointPerRangTournament.toString());
      expect(mytournamentFormBloc.pointStart.value, tournament.rangStartTournament.toString());
      //TODO test image
      // expect(tournamentFormBloc.imageCup.value, file);
      //TODO test cashprize
      // expect(mytournamentFormBloc.listCashPrize.value, tournament.cashPrize);
      //TODO test listCodeGame
      expect(mytournamentFormBloc.listCode.value[0].value, 'init');
  ...
  group('Normal test', (){
    setUp((){
      // when(() => tournament,).thenReturn(null);
    });
    test('changeCode', () {
      String monCode = 'MonCode';
      when(()=> generateCodeCubit.generateCode(any(), 5)).thenReturn(monCode);
      tournamentFormBloc.listCode.addFieldBloc(TextFieldBloc(initialValue: ''));
      tournamentFormBloc.changeCode(0);
      expect(tournamentFormBloc.listCode.value[0].value, monCode);
    });
    ...
    test('modifCashPrize', () {
      ...
      expect(tournamentFormBloc.listCashPrize.value[0], 'MaValeur');
    });
    test('addCashPrize', () {
  ...
  group('Bloc test', (){
    ...
    group('Step 2', (){
      setUp((){
        ...
        when(()=> generateCodeCubit.generateCode(any(), 5)).thenReturn('123456789123412345');
      });
      blocTest(
    ...
    group('Step 6', (){
      setUp(() {
        ...
        tournamentFormBloc.addFieldBlocs(fieldBlocs: [TextFieldBloc(validators: [FieldBlocValidatorsFr.required])],step: 6);
        tournamentFormBloc.updateCurrentStep(5);
        tournamentFormBloc.listCode.addFieldBloc(TextFieldBloc(validators: [FieldBlocValidatorsFr.required],initialValue: ''));
      });
      blocTest(
    ...
    blocTest(
      ...
      setUp: (){
        ...
        tournamentFormBloc.listCode.addFieldBloc(TextFieldBloc(validators: [FieldBlocValidatorsFr.required],initialValue: 'test'));
      },
      act: (TournamentFormBloc bloc) {
        bloc.submit();
      },
