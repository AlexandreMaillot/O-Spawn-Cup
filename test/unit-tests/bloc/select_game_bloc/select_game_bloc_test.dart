...
void main() {
  final list = <CardGame>[
    CardGame('img', GameName.csgo),
    CardGame('img', GameName.fornite),
    CardGame('img', GameName.leagueOfLegend),
  ];
  test('init bloc with initialindex', () {
    expect(
      SelectGameBloc(initialIndex: 2, listGameName: []).state,
      const SelectGameChanged(2),
    );
  });
  ...
  blocTest<SelectGameBloc, SelectGameState>(
    ...
    build: () => SelectGameBloc(initialIndex: 0, listGameName: list),
    act: (bloc) {
      bloc.add(const SelectGameChange(indexSelect: 1));
    },
    expect: () => [const SelectGameChanged(1)],
  );
}
